...
public class GlobalEnum {
    ...
    public enum Visibility {
        ...
        public static Visibility fromCode(int code) {
            ...
            throw new IllegalArgumentException("유효하지 않은 Visibility 코드: " + code);
        }

        /** 이름으로 파싱(대소문자 무시). 필요 없으면 삭제해도 OK */
        public static Visibility fromNameIgnoreCase(String name) {
            for (Visibility v : values()) {
                if (v.name().equalsIgnoreCase(name)) return v;
            }
            throw new IllegalArgumentException("유효하지 않은 Visibility 이름: " + name);
        }
    }
